...
public interface IRepositoryBase<TEntity, in TKey> where TEntity : class
{
    ...
    Task<TResult?> GetAsync<TResult>(Expression<Func<TEntity, bool>> expression, bool asNoTracking = false, CancellationToken cancellationToken = default);

    Task<TEntity?> GetAsync(ISpecification<TEntity> specification, CancellationToken cancellationToken = default);

    Task<TResult?> GetAsync<TResult>(ISpecification<TEntity> specification, CancellationToken cancellationToken = default);
